...
type: evaluation
objective: >-
  Produce reconciled final statistics, multi-corpus comparison, ceiling fraction reporting, regression confidence intervals,
  and calibration analysis—addressing all major and minor reviewer critiques.
approach: >-
  This evaluation depends on the new experiment above being available; it performs: (1) RECONCILE CNN/DM: recompute Table
  1 from the per-row data produced by the new experiment run; (2) MULTI-NEWS RESULTS: ROUGE table for TF and TF-IDF scorers
  at all theta/fixed-ratio levels; Wilcoxon Holm-corrected tests; compression ratio std — test whether std > 10pp; (3) CEILING
  FRACTION: for each theta on each corpus, % documents where percolation hits 100%; (4) REGRESSION with per-feature coefficient
  95% CIs and partial R²; (5) CALIBRATION ANALYSIS: what theta produces mean 10% compression and whether that is consistent
  across documents; (6) CROSS-CORPUS COMPARISON of compression ratio std CNN/DM vs Multi-News. Output eval_out.json with full
